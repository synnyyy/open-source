...
local RizzlerWindow = Window:CreateTab("Rizzler", 4483362458)
...
amountofpickuplines = 0

for _, v in ipairs(test) do
   amountofpickuplines = amountofpickuplines + 1
end

local PickUpLinesAmount = RizzlerWindow:CreateLabel("Pick up lines: " .. tostring(amountofpickuplines))





local TheLines = RizzlerWindow:CreateDropdown({
   Name = "Select your line",
   Options = test,
   CurrentOption = "Option 1",
   Flag = "Dropdown1", 
   Callback = function(Option)
        getgenv().line = tostring(Option[1])
   end,
})

local Chat = RizzlerWindow:CreateButton({
   Name = "Say line",
   Info = "Says the selected line",
   Interact = '??',
   Callback = function()
        ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer(getgenv().line, "All")
   end,
})
...
local Chat2 = RizzlerWindow:CreateButton({
   ...
   Interact = '??',
   Callback = function()
        ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer(test[math.random(1, #test)], "All")
   end,
})
